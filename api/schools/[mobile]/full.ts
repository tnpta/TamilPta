...
import { getAuthUser, hasRole } from '../../_lib/auth.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const [
      ...
      supabase.from('school_room_details').select('*').eq('school_mobile_no', mobileStr).order('serial_no'),
      supabase.from('school_classroom_grid').select('*').eq('school_mobile_no', mobileStr).order('block_no'),
