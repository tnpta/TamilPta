...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!data.basic?.school_name) {
    return res.status(400).json({ ok: false, error: 'School name is required for submission' });
  }

  try {
    // Save all data using the same pattern as draft, but with SUBMITTED status
    await saveSchoolData(mobileStr, data);
...
async function saveSchoolData(mobile: string, data: any) {
  // 1. Basic Info - always set status to SUBMITTED
  if (data.basic) {
  ...
  // 2-11: Same as draft.ts
  if (data.trust) {
    const t = data.trust;
    await supabase.from('trust_management_details').upsert({
      school_mobile_no: mobile,
      trust_name: t.trust_name || null, organization_type: t.organization_type || null,
      registration_date: t.registration_date || null, registration_number: t.registration_number || null,
      renewal_details: t.renewal_details || null, trust_document_path: t.trust_document_path || null,
      chairman_title: t.chairman_title || null, chairman_name: t.chairman_name || null,
      chairman_mobile_no: t.chairman_mobile_no || null, chairman_whatsapp_no: t.chairman_whatsapp_no || null,
      chairman_email_id: t.chairman_email_id || null,
      secretary_title: t.secretary_title || null, secretary_name: t.secretary_name || null,
      secretary_mobile_no: t.secretary_mobile_no || null, secretary_whatsapp_no: t.secretary_whatsapp_no || null,
      secretary_email_id: t.secretary_email_id || null,
      year_of_establishment: t.year_of_establishment || null, classes_at_establishment: t.classes_at_establishment || null,
      opening_order_no: t.opening_order_no || null, rc_no: t.rc_no || null, rc_date: t.rc_date || null,
  ...
  if (data.staff?.summary) {
    ...
    await supabase.from('staff_summary').upsert({
      school_mobile_no: mobile,
      teaching_pg_count: s.teaching_pg_count || 0, teaching_ug_count: s.teaching_ug_count || 0,
  ...
  if (data.staff?.nonTeaching) {
    ...
    await supabase.from('non_teaching_staff_summary').upsert({
      school_mobile_no: mobile,
      oa_count: n.oa_count || 0, driver_vehicle_helper_count: n.driver_vehicle_helper_count || 0,
      watchman_count: n.watchman_count || 0, aaya_count: n.aaya_count || 0,
      sweeper_count: n.sweeper_count || 0, others_count: n.others_count || 0,
    }, { onConflict: 'school_mobile_no' });
  }

  if (data.staff?.students?.length > 0) {
    await supabase.from('class_wise_student_strength').delete().eq('school_mobile_no', mobile);
    const rows = data.staff.students.filter((s: any) => s.class_name).map((s: any) => ({
      school_mobile_no: mobile, class_name: s.class_name,
      boys_count: s.boys_count || 0, girls_count: s.girls_count || 0,
      rte_count: s.rte_count || 0, emis_upload_path: s.emis_upload_path || null,
    }));
    if (rows.length > 0) await supabase.from('class_wise_student_strength').insert(rows);
  }

  if (data.infrastructure?.main) {
    const i = data.infrastructure.main;
    await supabase.from('school_infrastructure').upsert({
      school_mobile_no: mobile, school_location_type: i.school_location_type || null,
      total_land_acres: i.total_land_acres || null, total_land_sqft: i.total_land_sqft || null,
      ...
      land_latitude: i.land_latitude || null, land_longitude: i.land_longitude || null,
      land_ownership_type: i.land_ownership_type || null, lease_years: i.lease_years || null,
      ...
      toilets_boys_count: i.toilets_boys_count || 0, toilets_girls_count: i.toilets_girls_count || 0,
  ...
  if (data.infrastructure?.blocks?.length > 0) {
    ...
    const rows = data.infrastructure.blocks.filter((b: any) => b.block_no != null).map((b: any) => ({
      school_mobile_no: mobile, block_no: b.block_no,
      year_of_construction: b.year_of_construction || null, number_of_floors: b.number_of_floors || 0,
      is_building_plan_approved: !!b.is_building_plan_approved,
      approval_details: b.approval_details || null, approval_order_copy_path: b.approval_order_copy_path || null,
      floors_data: b.floors_data || null,
    }));
    if (rows.length > 0) await supabase.from('school_building_blocks').insert(rows);
  }

  if (data.infrastructure?.certificates?.length > 0) {
    await supabase.from('school_certificates').delete().eq('school_mobile_no', mobile);
    const rows = data.infrastructure.certificates.filter((c: any) => c.certificate_type).map((c: any) => ({
      school_mobile_no: mobile, certificate_type: c.certificate_type,
      certificate_no: c.certificate_no || null, date_authority: c.date_authority || null,
      persons_accommodated: c.persons_accommodated || null, certificate_document_path: c.certificate_document_path || null,
    }));
    if (rows.length > 0) await supabase.from('school_certificates').insert(rows);
  }
  ...
  if (data.fees?.vehicles) {
    ...
    await supabase.from('school_vehicles').upsert({
      school_mobile_no: mobile,
      number_of_vans: v.number_of_vans || 0, number_of_buses: v.number_of_buses || 0,
  ...
  if (data.fees?.others) {
    ...
    await supabase.from('school_other_compliance_details').upsert({
      school_mobile_no: mobile,
      has_health_care_center: !!o.has_health_care_center, health_care_details: o.health_care_details || null,
