...
async function saveSchoolData(mobile: string, data: any, isSubmit: boolean) {
  ...
  // 12. Room Details (multi-row: delete + insert)
  if (data.infrastructure?.roomDetails?.length > 0) {
    ...
    const rows = data.infrastructure.roomDetails
      ...
      .map((r: any, idx: number) => ({
        ...
        serial_no: r.serial_no || idx + 1,
        num_rooms: r.num_rooms || 0,
        block: r.block || null,
        floor: r.floor || null,
        area_sqft: r.area_sqft || null,
        roofing: r.roofing || null,
      }));
    if (rows.length > 0) {
      await supabase.from('school_room_details').insert(rows);
    }
  }

  // 13. Classroom Grid (multi-row: delete + insert)
  if (data.infrastructure?.classroomGrid?.length > 0) {
    ...
    const rows = data.infrastructure.classroomGrid
      ...
      .map((g: any) => ({
        school_mobile_no: mobile,
        block_no: g.block_no,
        ground_floor_rooms: g.ground_floor_rooms || 0,
        first_floor_rooms: g.first_floor_rooms || 0,
        second_floor_rooms: g.second_floor_rooms || 0,
        third_floor_rooms: g.third_floor_rooms || 0,
      }));
    if (rows.length > 0) {
      await supabase.from('school_classroom_grid').insert(rows);
    }
  }
}
